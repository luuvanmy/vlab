...
(function($, window, document, undefined) {
    ...
    var defaults = {
        ...
        emptyClass: 'dd-empty',
        expandBtnHTML: '<button class="dd-expand dd_btn" data-action="expand" type="button">Expand</button>',
        collapseBtnHTML: '<button class="dd-collapse dd_btn" data-action="collapse" type="button">Collapse</button>',
        group: 0,
        maxDepth: 5,
        threshold: 20,
    ...
    Plugin.prototype = {
        ...
        init: function() {
            ...
            var onStartEvent = function(e) {
               var handle = $(e.target);
               if(handle.is('.btn-action , .btn-action *')){
                	list.isTouch = /^touch/.test(e.type);
	                if (list.isTouch && e.touches.length !== 1) {
	                    return;
	                }
                	var  action = handle.data('action');
                	var id = handle.data('id');
                	if( action == 'remove' || action == 'edit' ){
                		list.dragStart(e.touches ? e.touches[0] : e ,action , id);
                	}else{
                		list.dragStart(e.touches ? e.touches[0] : e , null , null);
                	}
                	return false;
                }
        ...
        dragStart: function(e , action , itemId) {
            ...
            if( action == 'remove' ||  action == 'edit'){
            	 continueExecution = this.options.onDragStart.call(this, this.el, dragItem, position , action , itemId);
          	}else{
          		 continueExecution = this.options.onDragStart.call(this, this.el, dragItem, position , null , null);
          	}
