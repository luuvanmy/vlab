            ...
            if( vm.options.chooseNumber ){
                ...
                $.post("/admin/sales/pos/postAddProductToTable", {
                    ...
                    if (res.error == false) {
                        ...
                        console.log("adjasdhagda");
                        if(res.hasOwnProperty('data_send_chief')){
                            console.log("kakakaka");
                            socket.emit('pos_user_send_success' , {
                                data : res.data_send_chief
                            });
                        }
                        ...
                        buttons: {
                            save : {
                                ...
                                action : ()=>{
                                    ...
                                    $.post('/admin/sales/pos/postCheckBookingTable',info, (res) => {
                                        if (res.status == 403) return;
                                        if (res.error == false) {
        ...
        <div class="item-product-order detail_source_product">
            ...
            <div class="item-product-quantity">
                ...
                <div class="item-quantity-info" >
                    <span class="make-success">{{item.recive_quantity}}</span>/<span class="chief-quantity">{{item.request_quantity}}</span>
                </div>
            </div>
            <div class="item-product-price">
                ...
                if (res.error == false) {
                    ...
                    if(res.hasOwnProperty('data')){
                        if(!res.data.processing){
                            this.item.request_quantity = res.data.request_quantity;
                            this.item.recive_quantity = res.data.recive_quantity;
                        }
                    }
                }
            }).fail(function () {
        ...
        methods: {
            ...
            showModalJoinTable : function(id){
                ...
                var tables = _.filter(this.sources, function (item) {
                    var checkEmpty = true;
                    if (item.state == 'booked' && item.relation_id != '') {
                        checkEmpty = false;
                    }
                        ...
                        save : {
                            ...
                            action : ()=>{
                                ...
                                return new Promise((resolve, reject)=>{
                                    ...
                                    $.post("/admin/sales/pos/postCheckMergeSaleSource", {
                                        id_source: id,
                                        id_target: target,
                                    }, function (res) {
                                        if (res.status == 403) return;
                                        ...
                                        if(res.error){
                                            $.confirm({
                                                title : 'Thông báo ',
                                                content : res.message+', Bạn vẫn muốn gộp bàn?',
                                                buttons :{
                                                    oke : {
                                                        text : 'Đồng ý',
                                                        btnClass : 'btn-success',
                                                        action : ()=>{
                                                            $.post("/admin/sales/pos/postMergeSaleSource", {
                                                                ...
                                                                if (res.error == false) {
                                                                    ...
                                                                    if(res.hasOwnProperty('list_user_socket')){
                                                                        socket.emit('pos_list_user_reload_wait',res.list_user_socket);
                                                                    }
        ...
        mounted: function () {
            ...
            socket.on('pos_on_send_success', function(res){
                if( vm.config.tab == 'wait'){
                    if(vm.user._id == res.data.user_id || vm.user.type_pos =='manager'){
                        vm.getListSuccessChief();
                    }
            ...
            socket.on('pos_on_supply', function(res){
                console.log(res);
                if( vm.config.tab == 'wait'){
                    if(vm.user._id == res.user._id || vm.user.type_pos =='manager'){
                        vm.getListSuccessChief();
                    }
                    
                }
            }); 
            socket.on('pos_on_reload_wait', function(res){
                if( vm.config.tab == 'wait'){
                    if(res.indexOf(vm.user._id) >= 0 || vm.user.type_pos =='manager'){
                         vm.getListSuccessChief();
                    } 
                }
            });
